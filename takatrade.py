...
st.markdown("""
    ...
    .logo-container {
        ...
        padding: 20px 0; letter-spacing: 4px; border-bottom: 1px solid #1a1a1a; margin-bottom: 20px;
    }
    div[data-testid="stMetric"] { background: #0a0a0a; border: 1px solid #222; padding: 20px; border-radius: 15px; }
    .stButton>button { 
        background: linear-gradient(45deg, #FFD700, #FF8C00); color: #000; border: none; font-weight: 800; 
        border-radius: 10px; width: 100%; height: 3.8em; transition: all 0.3s ease;
    }
    .stButton>button:hover { transform: translateY(-2px); box-shadow: 0 5px 15px rgba(255, 215, 0, 0.3); }
    .news-card { background: #0d0d0d; border-left: 4px solid #FFD700; padding: 15px; margin-bottom: 12px; border-radius: 8px; border: 1px solid #222; }
    .status-box { text-align:center; padding:20px; background: rgba(10,10,10,0.8); border-radius:15px; margin-bottom:25px; border: 1px solid #333; }
    </style>
...
def train_ai_engine(ticker, interval, period, epochs):
    ...
    try:
        ...
        # PERBAIKAN: Inverse transform membutuhkan 3 kolom input sesuai dengan scaler
        res = scaler.inverse_transform([[p_scaled, scaled_data[-1, 1], scaled_data[-1, 2]]])[0, 0]
        return df, res
...
if selected == "Intelligence":
    ...
    if st.button("RUN DEEP ANALYSIS"):
        ...
        for i, t in enumerate(selected_assets):
            with tabs[i]:
                with st.spinner(f'Synchronizing {t}...'):
                    ...
                    if df is not None:
                        ...
                        """, unsafe_allow_html=True)
                        fig = go.Figure(data=[go.Candlestick(x=df.index[-60:], open=df['Open'], high=df['High'], low=df['Low'], close=df['Close'], increasing_line_color='#00FFCC', decreasing_line_color='#FF4B4B')])
                        ...
                        if news_list:
                            with st.expander("📰 Intelligence Feed"):
                                for n in news_list[:3]:
                                    st.markdown(f"""<div class="news-card"><small style="color:#FFD700;">Market Alert</small><br><b style="font-size:14px;">{n.get('title', 'Headline')}</b><br><a href="{n.get('link', '#')}" target="_blank" style="color:#00FFCC; text-decoration:none; font-size:11px;">View Source →</a></div>""", unsafe_allow_html=True)
                    else: st.error(f"Analysis failed for {t}.")
    ...
    if st.button("INITIATE SECTOR SCAN"):
        ...
        if results:
            ...
            st.dataframe(res_df.style.map(lambda x: 'color: #00FFCC' if x == 'BUY' else ('color: #FF4B4B' if x == 'SELL' else 'color: #FFA500'), subset=['Signal']), use_container_width=True)
