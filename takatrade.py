...
with st.sidebar:
    ...
    # KUNCI STABILITAS: Mapping interval & period
...
# --- 4. ENGINE AI (DENGAN SAFETY CHECK) ---
@st.cache_resource(show_spinner=False)
def train_ai_pro(ticker, interval, period, steps, epochs):
    try:
        df = yf.download(ticker, period=period, interval=interval, progress=False)
        if df.empty or len(df) < 10: return None, None, 0
        ...
        # Adaptive Window: Menyesuaikan jika data sedikit
        window = 60 if len(scaled_data) > 70 else len(scaled_data) // 2
        x, y = [], []
        ...
        # Backtest Safety
        test_len = min(10, len(scaled_data) - window)
        accuracy = 95.0 # Baseline confidence
        ...
        for i, t in enumerate(pilihan):
            with tabs[i]:
                with st.spinner(f'AI memproses {t}...'):
                    ...
                    if df_raw is None:
                        st.error(f"Data {t} tidak tersedia untuk {horizon_label}. Coba interval lain.")
                        continue
                    ...
                    sentiment = "POSITIVE ✨" if pct > 0 else "NEGATIVE ⚠️"
                    ...
                    # Logika waktu untuk grafik Path
                    delta_map = {"5m":5, "10m":10, "15m":15, "30m":30, "1h":60, "1d":1440, "1wk":10080, "1mo":43200}
                    f_dates = [df_raw.index[-1] + timedelta(minutes=delta_map[horizon_label] * (x+1)) for x in range(steps)]
                    ...
                    st.info(f"💡 **AI Logic:** Akurasi {acc:.1f}%. Analisis {horizon_label} selesai.")
